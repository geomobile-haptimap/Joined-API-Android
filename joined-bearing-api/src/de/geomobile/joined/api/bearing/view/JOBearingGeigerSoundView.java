 ...
 * 
 */
public class JOBearingGeigerSoundView extends ImageView implements JOBearingRotationImage
{
	 ...
	 * @param mp
	 */
	public JOBearingGeigerSoundView(Context context, AttributeSet attrs, int defStyle, MediaPlayer mp)
	{
	 ...
	 * @param attrs
	 */
	public JOBearingGeigerSoundView(Context context, AttributeSet attrs)
	{
		super(context, attrs);
		this.context = context;
	}
	 ...
	 * @param context
	 */
	public JOBearingGeigerSoundView(Context context)
	{
		super(context);
		this.context = context;
	}
	...
	{
		LinearLayout soundButton = (LinearLayout) ((Activity) context).findViewById(R.id.soundButton);
		LinearLayout vibrationButton = (LinearLayout) ((Activity) context).findViewById(R.id.vibrationButton);
		if (flag)
		{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.geomobile.joined.api.bearing.JOBearingRotationImage#rotate(float)
	 */
	public void rotate(float degree)
	{
		if (degree <= 90 && degree >= -90)
		{
			soundPause = 580 + (int) (Math.cos(degree * Math.PI / 180 + Math.PI) * 500);
		}
		else
		{
		...
		vibSoundThread = new Thread()
		{
			...
			public void run()
			{
				while (isActive)
				{
					...
					while (isActive && System.currentTimeMillis() - currentTime < soundPause)
					{
						try
						{
							sleep(10);
			...
			public void doAction()
			{
				new Thread()
				{
					public void run()
					{
						...
						if (rotationAngle > 35 && rotationAngle < 58 && hasBeenMoved)
						{
							vibrator.vibrate(200);
						}
						else if (rotationAngle > 30 && rotationAngle < 58 && hasBeenMoved)
						{
							vibrator.vibrate(65);
						}
						else if (rotationAngle > 25 && rotationAngle < 58 && hasBeenMoved)
						{
							vibrator.vibrate(55);
						}
						else if (rotationAngle > 20 && rotationAngle < 58 && hasBeenMoved)
						{
							vibrator.vibrate(40);
						}
						else if (rotationAngle > 15 && rotationAngle < 58 && hasBeenMoved)
						{
							vibrator.vibrate(30);
						}
						else if (rotationAngle > 0 && rotationAngle < 58 && hasBeenMoved)
						{
							vibrator.vibrate(25);
						}
					};
